...
INPUT_FILE = PROJECT_ROOT / 'data' / 'figure_inputs' / 'CN_ratio_map_values.xlsx'
OUTPUT_DIR = PROJECT_ROOT / 'outputs' / 'figures' / 'province_maps'
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
OUTPUT_MAP = OUTPUT_DIR / 'CN_ratio_map.html'
OUTPUT_PNG = OUTPUT_DIR / 'CN_ratio_ranking.png'
OUTPUT_PDF = OUTPUT_DIR / 'CN_ratio_ranking.pdf'
# ================================
...
try:
    ...
    map_data = list(zip(df['Eng_Province'], df[ch4_col]))
    ...
    map_chart = (
        Map(init_opts=opts.InitOpts(width="1400px", height="900px", theme=ThemeType.LIGHT,
                                    page_title="Premium CH4 Map"))
        .add(
            series_name="CH4 Rate",
            data_pair=map_data,
            maptype="china",
        ...
        .set_global_opts(
            ...
            visualmap_opts=opts.VisualMapOpts(
                min_=df[ch4_col].min(), max_=df[ch4_col].max(),
...
try:
    ...
    df_sorted = df.sort_values(ch4_col, ascending=True)

    premium_cmap = LinearSegmentedColormap.from_list("premium_red", PREMIUM_COLORS)
    norm = plt.Normalize(vmin=df_sorted[ch4_col].min(), vmax=df_sorted[ch4_col].max())

    bars = ax.barh(range(len(df_sorted)), df_sorted[ch4_col],
                   color=premium_cmap(norm(df_sorted[ch4_col])), height=0.65)
    ...
    ax.set_title('Provincial CH4 Emissions Ranking', fontsize=16, fontfamily='Arial', fontweight='bold', color='#222',
                 pad=25, loc='left')

    for i, (idx, row) in enumerate(df_sorted.iterrows()):
        ax.text(row[ch4_col] + (df[ch4_col].max() * 0.02), i, f'{row[ch4_col]:.3f}',
