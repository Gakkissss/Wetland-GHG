# -*- coding: utf-8 -*-
# @Time    : 2026/4/16 19:18
# @Author  : 55050
# @File    : model_performance_N2O.py
# @Software: PyCharm
...
# 引入 ExtraTreesRegressor 防止未导入报错
from sklearn.ensemble import ExtraTreesRegressor

# ================= 1. 路径与全局变量配置 (针对 N2O ET模型) =================
PROJECT_ROOT = Path(__file__).resolve().parents[2]
DATA_PATH = PROJECT_ROOT / 'data' / 'model_inputs' / 'N2O_model_input.xlsx'
BEST_MODEL_PATH = PROJECT_ROOT / 'models' / 'N2O_ET_model.joblib'
OUTPUT_PATH = PROJECT_ROOT / 'outputs' / 'figures' / 'N2O_model_performance.png'
OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)

SELECTED_FEATURES = ['TN_re', 'CN_in', 'NH4_re', 'Depth', 'NH4_in', 'NO3_re', 'TN_in', 'NO3_in']
RANDOM_STATE_TARGET = 274
...
def main():
    ...
    y = df['N2O'].copy()  # 注意这里改为了 N2O

    # 1%~99% 分位裁剪 N2O (去除极端异常值)
    ...
    if isinstance(best_model, Pipeline):
        ...
        # 【修复的Bug】：不再强制检查是否为 XGBRegressor，只要模型有 predict 方法即可
        if not hasattr(best_model, 'predict'):
            raise TypeError("加载的模型没有 predict 方法，请确认 ET_best_model.joblib 的内容。")
    ...
    # 坐标轴标签 (加入了规范的 N2O 化学式 LaTeX)
    ax.set_xlabel('Measured N$_2$O Emission', fontsize=13, fontweight='bold')
    ax.set_ylabel('Predicted N$_2$O Emission', fontsize=13, fontweight='bold')
    ax.set_xlim(axis_range)
    ax.set_ylim(axis_range)

    # 图表标题 (标明是 ET 模型的表现)
    ax.set_title('ET Model Performance: Predicted vs. Measured N$_2$O', fontsize=14, fontweight='bold', pad=15)
