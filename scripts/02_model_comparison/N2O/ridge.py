...
n_runs = 50
records = []
print(f"开始进行 {n_runs} 次 Ridge 基准测试 (含每次CV网格搜索)...")
...
for run_seed in range(n_runs):
    ...
    # 模型与网格搜索
    model_ridge = Ridge(random_state=run_seed, max_iter=10000)
    grid_search_ridge = GridSearchCV(model_ridge, param_grid_ridge, scoring='r2', cv=5, n_jobs=-1, verbose=0)
    grid_search_ridge.fit(X_train, y_train)

    best_params = grid_search_ridge.best_params_
    cv_best_r2 = grid_search_ridge.best_score_
...
output_path = OUTPUT_DIR / 'Ridge_benchmark_results.xlsx'
