...
n_runs = 50
records = []
print(f"开始进行 {n_runs} 次 GBM 基准测试 (含每次CV网格搜索)...")
...
for run_seed in range(n_runs):
    ...
    # 模型与网格搜索
    base_params = {'random_state': run_seed}
    model_gbm = GradientBoostingRegressor(**base_params)
    grid_search_gbm = GridSearchCV(model_gbm, param_grid_gbm, scoring='r2', cv=5, n_jobs=-1, verbose=0)
    grid_search_gbm.fit(X_train, y_train)

    best_params = grid_search_gbm.best_params_
    cv_best_r2 = grid_search_gbm.best_score_
...
output_path = OUTPUT_DIR / 'GBM_benchmark_results.xlsx'
