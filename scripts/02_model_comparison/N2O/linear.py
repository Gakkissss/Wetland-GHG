...
n_runs = 50
records = []
print(f"开始进行 {n_runs} 次 LinearRegression 基准测试 ...")
...
for run_seed in range(n_runs):
    ...
    cv_scores = cross_val_score(model_lin, X_train, y_train, cv=5, scoring='r2', n_jobs=-1)
    cv_best_r2 = np.mean(cv_scores)
...
output_path = OUTPUT_DIR / 'Linear_benchmark_results.xlsx'
