...
n_runs = 50
records = []
print(f"开始进行 {n_runs} 次 Lasso 基准测试 (含每次CV网格搜索)...")
...
for run_seed in range(n_runs):
    ...
    # 模型与网格搜索
    model_lasso = Lasso(random_state=run_seed, max_iter=10000)
    grid_search_lasso = GridSearchCV(model_lasso, param_grid_lasso, scoring='r2', cv=5, n_jobs=-1, verbose=0)
    grid_search_lasso.fit(X_train, y_train)

    best_params = grid_search_lasso.best_params_
    cv_best_r2 = grid_search_lasso.best_score_
...
output_path = OUTPUT_DIR / 'Lasso_benchmark_results.xlsx'
