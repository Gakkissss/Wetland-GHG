...
n_runs = 50
records = []
print(f"开始进行 {n_runs} 次 Decision Tree 基准测试 (含每次CV网格搜索)...")
...
for run_seed in range(n_runs):
    ...
    # 网格搜索
    model_dt = DecisionTreeRegressor(random_state=run_seed)
    grid_search_dt = GridSearchCV(model_dt, param_grid_dt, scoring='r2', cv=5, n_jobs=-1, verbose=0)
    grid_search_dt.fit(X_train, y_train)

    best_params = grid_search_dt.best_params_
    cv_best_r2 = grid_search_dt.best_score_
