...
n_runs = 50
records = []
print(f"开始进行 {n_runs} 次 SVR 基准测试 (含每次CV网格搜索)...")
...
for run_seed in range(n_runs):
    ...
    # 模型与网格搜索
    model_svr = SVR()
    grid_search_svr = GridSearchCV(model_svr, param_grid_svr, scoring='r2', cv=5, n_jobs=-1, verbose=0)
    grid_search_svr.fit(X_train, y_train)

    best_params = grid_search_svr.best_params_
    cv_best_r2 = grid_search_svr.best_score_
...
output_path = OUTPUT_DIR / 'SVR_benchmark_results.xlsx'
