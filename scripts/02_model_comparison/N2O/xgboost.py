...
n_runs = 50
records = []
print(f"开始进行 {n_runs} 次 XGBoost 基准测试 (含每次CV网格搜索)...")
...
for run_seed in range(n_runs):
    ...
    grid_search_xgb = GridSearchCV(model_xgb, param_grid_xgb, scoring='r2', cv=5, n_jobs=-1, verbose=0)
...
output_path = OUTPUT_DIR / 'XGB_benchmark_results.xlsx'
