...
try:
    ...
    n_runs = 50
    records = []
    print(f"开始进行 {n_runs} 次 CatBoost 基准测试 (含每次CV网格搜索)...")
    ...
    for run_seed in range(n_runs):
        ...
        model_cat = CatBoostRegressor(**params_cat_base)
        grid_search_cat = GridSearchCV(model_cat, param_grid_cat, scoring='r2', cv=5, n_jobs=-1, verbose=0)
        grid_search_cat.fit(X_train, y_train)

        best_params = grid_search_cat.best_params_
        cv_best_r2 = grid_search_cat.best_score_
    ...
    output_path = 'CatBoost_benchmark_results.xlsx'
    ...
    print('未安装 catboost，跳过 CatBoost 模型。pip install catboost 可启用。')
