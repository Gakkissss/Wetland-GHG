...
n_runs = 50
records = []
print(f"开始进行 {n_runs} 次 RF 基准测试 (含每次CV网格搜索)...")
...
for run_seed in range(n_runs):
    ...
    model_rf = RandomForestRegressor(**params_rf_base)
    grid_search_rf = GridSearchCV(model_rf, param_grid_rf, scoring='r2', cv=5, n_jobs=-1, verbose=0)
    grid_search_rf.fit(X_train, y_train)

    best_params = grid_search_rf.best_params_
    cv_best_r2 = grid_search_rf.best_score_
...
output_path = OUTPUT_DIR / 'RF_benchmark_results.xlsx'
