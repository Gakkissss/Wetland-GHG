...
MODEL_PATH = PROJECT_ROOT / 'models' / 'CH4_XGBoost_model.joblib'
META_PATH = PROJECT_ROOT / 'models' / 'CH4_XGBoost_metadata.json'
DATA_PATH = PROJECT_ROOT / 'data' / 'model_inputs' / 'CH4_model_input.xlsx'
OUTPUT_DIR = PROJECT_ROOT / 'outputs' / 'figures' / 'SHAP' / 'CH4'
...
def load_model_and_meta():
    if not MODEL_PATH.exists() or not META_PATH.exists():
        raise FileNotFoundError(
            f"未找到模型或元数据文件:\n  {MODEL_PATH}\n  {META_PATH}\n请先运行 Causal_XGBoost_Optimize.py 生成这些文件。"
        )
...
def save_figures(shap_values, X_proc, feature_names, global_importance):
    ...
    shap.summary_plot(
        ...
        max_display=8,  # 限制只显示前8个特征
        show=False
    )
    plt.tight_layout()
    path_summary = OUTPUT_DIR / 'XGB_shap_summary_top8.pdf' # 推荐导出为PDF矢量图用于顶刊排版
    ...
    plt.tight_layout()
    path_bar = OUTPUT_DIR / 'XGB_shap_bar_top8.pdf' # 推荐导出为PDF矢量图
    ...
    for idx in top_idx:
        ...
        plt.tight_layout()
        path_dep = OUTPUT_DIR / f"XGB_shap_dependence_{feat}_top8.pdf"
...
def export_shap_csv(shap_values, X_proc, feature_names, pipe: Pipeline, X_raw: pd.DataFrame, y=None):
    ...
    if y is not None:
        df_out.insert(0, 'CH4', y.values)

    out_path = OUTPUT_DIR / 'XGB_shap_values_top8.csv'
...
def main():
    ...
    imp_path = OUTPUT_DIR / 'XGB_shap_importance_top8.csv'
